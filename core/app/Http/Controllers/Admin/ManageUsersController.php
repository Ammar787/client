...
class ManageUsersController extends Controller
{
    ...
    public function userServiceBooking($id)
    {
        $user = User::findOrFail($id);
        $pageTitle = 'User service booking - ' . $user->username;
        $emptyMessage = 'No data found.';

        $bookings = Booking::where('user_id', $user->id)->whereNotNull('service_id')->where('status', '!=', '0')->latest()->paginate(getPaginate());
